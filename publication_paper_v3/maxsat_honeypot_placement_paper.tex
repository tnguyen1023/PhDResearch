...
\subsection{Why RC2 wins: the force-multiplier mechanism}
This mechanism explains \emph{why} RC2 outperforms greedy placement, not merely \emph{that} it does (Fig.~\ref{fig:fm}). A single deployment simultaneously satisfies soft clauses across every attack path that intersects its hop, while a greedy solver -- limited to the single highest-$\rho$ path at each step -- leaves the remaining credit unclaimed. On the representative shared-hop slice: one \texttt{db\_trap} in the Internal zone earns $0.700+0.330+0.415=1.445$ in joint L3 credit across three concurrent paths, versus $0.700$ for a greedy solver considering only the highest-probability path -- a $2.06\times$ force multiplier, matching the base formulation's theoretical $\approx\!2.1\times$ (Fig.~\ref{fig:fm}, bottom row: FM-avg~$=1.53\times$, FM-max~$=1.90\times$ across all deployed types at Scale=XLarge, with the certified worst-case floor $r^*$ leading the best baseline by $+26.2\%$). Critically, this ratio is \emph{preserved, not created,} by the persona prior: after a STIX-driven shift elevates $q_{p}(\text{Finance\_DB})$, absolute credit rises (Fig.~\ref{fig:fm}, bottom-right) but the RC2-to-greedy ratio holds, confirming the force multiplier is a structural property of the WCNF encoding -- joint satisfiability of shared-hop soft clauses -- rather than an artifact of the persona weighting layer.
...
\underbrace{r^*(x)=\min_{\theta\in\Theta}\,Q_\theta(x)}_{\text{worst-case floor (16)--(17)}}
\;\longrightarrow\;
\text{RC2 maximises }r^*
\end{equation*}
...
The single most important structural property of the formulation is that one deployment decision simultaneously satisfies soft clauses across every attack path that shares its hop:
\[
\mathrm{FM}_{\mathrm{effective}} = \frac{\sum_\pi \rho_\pi\cdot iv_{\pi,h}\cdot W_{j,a}\cdot q_p}{\max_\pi\;\rho_\pi\cdot iv_{\pi,h}\cdot W_{j,a}\cdot q_p}
\]
For a \texttt{db\_trap} in the Internal zone at $q_p{=}0.40$: three concurrent paths yield credit $0.700{+}0.330{+}0.415{=}1.445$ versus $0.700$ for a greedy solver that sees only the highest-$\rho$ path -- a $2.06\times$ force multiplier, matching the theoretical $\approx2.1\times$. The ratio is \emph{preserved, not created}, by the persona prior: after a STIX-driven shift to $q_p(\text{Finance\_DB}){=}0.40$, absolute credit rises by 53\% while the FM ratio holds, confirming the multiplier is a property of the WCNF topology, not the weight layer.
