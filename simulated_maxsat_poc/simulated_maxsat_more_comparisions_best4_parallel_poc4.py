...
class MaxSAT:
    ...
    def solve(assets, budget):
        ...
        if not ILP_AVAILABLE:
            return MaxSAT._greedy(assets, honeypots, budget, start)

        try:
            model = LpProblem("MaxSAT", LpMaximize)
            x = {hp.id: LpVariable(f"x_{hp.id}", cat=LpBinary) for hp in honeypots}

            # COST-AWARE OBJECTIVE: Maximize value per dollar!
            obj = []
            for hp in honeypots:
    ...
    @staticmethod
    def _greedy(assets, honeypots, budget, start):
        scores = []
        for hp in honeypots:
            app = sum(1 for a in assets if a.asset_type in hp.applicable_asset_types)
            avg_c = np.mean([a.criticality for a in assets if a.asset_type in hp.applicable_asset_types]) if app > 0 else 0.8
            ...
            total_value = (0.5 * quality + 0.5 * coverage_value / 10) * avg_c * app
            value_per_dollar = total_value / hp.cost if hp.cost > 0 else 0
            scores.append((value_per_dollar, hp))
