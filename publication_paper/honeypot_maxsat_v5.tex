...
%  Student-Level Writing + Full Formulation Examples — v5
%  Related Work: Novelty Analysis + Gap Figure ADDED
% ============================================================
...
% ── helper macro: one data row ───────────────────────────────
% usage: \datarow{anchor}{label}{c1}{c2}{c3}{c4}{c5}{rowstyle}{cellstyle}
% We build rows manually for clarity.

% ── Row 1: Game-theoretic ────────────────────────────────────
...
The dashed line separates the best existing baseline from our
method.}
...
We now explain precisely where each body of prior work falls
short and why combining all five capabilities is a
...
The solution concept they use---Nash equilibrium---is also
fundamentally different from a proven weighted optimum: a
Nash equilibrium is a stable strategy pair, not the best
possible placement under a multi-objective scoring function.
...
means they cannot discover the cross-path force-multiplier
effect that makes our solver $2.1\times$ more efficient on
technique coverage (see cross-path analysis below).
...
constraints~(D4), and ILP provides a proven
optimum~(D5).
However, their model does not encode kill-chain step ordering
or early-interception priority~(D2), does not integrate
ATT\&CK technique weights or tactic-family bonuses~(D3),
...
network in 30~seconds by splitting the instance into five
parallel zone sub-problems.

\textbf{ATT\&CK red-team emulation (\xmark{}D4,
\xmark{}D5).}
Applebaum \etal{}~\cite{applebaum2016} show that ATT\&CK
can drive automated attack simulation.
Their framework covers D2 (kill-chain-structured paths) and
D3 (ATT\&CK techniques), but it is a red-team tool, not a
placement optimiser.
It does not enforce budget or conflict constraints~(D4) and
provides no optimality guarantee~(D5).

\textbf{Topology and attack-graph analysis (\xmark{}D3,
\xmark{}D4, \xmark{}D5).}
Jajodia \etal{}~\cite{jajodia2005} introduced the idea of
exploiting network chokepoints.
Their structural insight directly motivates our path-weight
formula
$PW_{\pi,h,j,a} = \rho_\pi \times iv_{\pi,h} \times W_{j,a}$.
However, they do not provide a placement algorithm, do not
integrate ATT\&CK~(D3), do not enforce constraints~(D4),
and do not prove optimality~(D5).

\textbf{MaxSAT for intrusion detection
(\xmark{}D1, \xmark{}D2, \xmark{}D3).}
Pinisetty \etal{}~\cite{pinisetty2017} demonstrated that
\maxsat{} is a practical tool for security rule selection.
However, their IDS rule-selection problem has no zone
structure~(D1), no kill-chain ordering~(D2), and no ATT\&CK
coverage objective~(D3).
Their work establishes the feasibility of \maxsat{} for
security resource allocation but does not address the
multi-zone honeypot placement problem.

\textbf{Moving-target defence (\xmark{}D2, \xmark{}D3,
\xmark{}D4, \xmark{}D5).}
Zhu and Ba\c{s}ar~\cite{zhu2013} address the complementary
problem of continuously re-configuring real assets to confuse
attackers.
MTD work is zone-aware at a routing level~(D1) but does not
address honeypot placement, kill-chain ordering~(D2),
ATT\&CK~(D3), conflict constraints~(D4), or
optimality~(D5).

\textbf{Greedy-BiDir baseline (\xmark{}D4, \xmark{}D5).}
Our strongest baseline is the first method we found that
combines D1 and D2 with partial D3 support.
It still misses D4 (no conflict constraints) and D5 (no
optimality guarantee), and it evaluates paths one at a time,
missing the cross-path overlap that allows our solver to earn
3.612 total weight from a single SSH honeypot deployment
versus greedy's 1.749.

\subsubsection{Cross-Path Force-Multiplier: The Key Technical
              Insight}

The most important reason why our method outperforms all
baselines is not just its optimality certificate---it is a
fundamental difference in how decisions are evaluated.
Consider technique T1021 (Remote Services) appearing at
hop~2 of three different attack paths simultaneously:
...
\end{tabular}
\end{center}
\textit{Deploying a single \texttt{ssh\_trap} in zone2 sets
$c_{\text{T1021},B}=1$, satisfying all three Level-3
clauses simultaneously.
Our solver counts the full weight 3.612; greedy counts only
1.749.
This \textbf{2.1$\times$} advantage per decision explains
why we outperform greedy methods on TechCov and FwdPath
by 100\% and 22\% respectively.}
\end{examplebox}

This cross-path force-multiplier effect is a direct
consequence of encoding all attack paths as clauses from the
start of the search.
No greedy method can discover it because greedy methods
evaluate objectives one at a time.

\subsection{How Our Work Compares}
\label{subsec:compare}

Table~\ref{tab:related} summarises the comparison at a glance.
Our method is the only one that satisfies all five capability
...
\begin{figure*}[!t]
  ...
  \caption{Network model.
           Panel~A: five-zone topology with directed
           connections, edge weights (traversal cost), and
           air-gap isolation between zone4/zone5 and the
...
Figure~\ref{fig:asset_and_coverage} (presented in
Section~\ref{sec:methodology} alongside the relevant
experimental tables) shows two structural properties of the
input.
...
T1046 Network Scan  & 0.65 & 0.1 & 1.1 &
  $0.65\!\times\!1.1=\mathbf{0.72}$ \\
T1048 Exfiltration  & 2.60 & 0.9 & 1.9 &
  $2.60\!\times\!1.9=\mathbf{4.94}$ \\
\bottomrule
\end{tabular}
\end{center}
\textit{Detecting exfiltration is worth $\mathbf{6.9}\times$
more than detecting a network scan.}
\end{examplebox}
...
web\_to\_db, hop3, T1048  & 0.30 & 2.0 & 4.94 &
  $\mathbf{2.96}$ (highest) \\
web\_to\_db, hop1, T1190  & 0.30 & 1.5 & 3.64 & $\mathbf{1.64}$ \\
ransomware, hop2, T1021   & 0.10 & 1.7 & 2.34 &
  $\mathbf{0.40}$ (lowest) \\
\bottomrule
\end{tabular}
\end{center}
\textit{The Level-4 early-intercept bonus pushes the solver
to cover hop~1 or~2 of web\_to\_db first, even though hop~3
has the highest $PW$---prevention beats forensics.}
\end{examplebox}
...
\begin{figure*}[!t]
  ...
  \caption{Detection coverage matrix (Panel~E).
           ...
           Techniques with high $\sigma$ (\eg{} T1048
           Exfiltration) receive high $W_{j,a}$ but appear
           in few zones---reflecting their difficulty of
           detection and the value of honeypots as the only
           viable sensor.}
...
\begin{examplebox}
\textbf{Example --- C1 for (T1021, SSH server B):}\\
Only \texttt{ssh\_trap} has $R[\text{T1021,ssh,sshB}]=1$.
C1 generates: $\neg c_{\text{T1021},B}
\vee x_{\texttt{ssh\_trap}}$.
Without this clause, the solver could claim perfect coverage
...
\begin{examplebox}
\textbf{Cross-Path Overlap --- The Key Force Multiplier:}\\
Deploying \texttt{ssh\_trap} in zone2 satisfies three
Level-3 clauses simultaneously:
$1.749 + 0.826 + 1.037 = \mathbf{3.612}$
vs.\ greedy's $1.749$ --- a $\mathbf{2.1\times}$ advantage.
\end{examplebox}

\subsubsection{Level 2: Cover Techniques and Tactic Families}

Per-technique: $\;\text{weight}_j = \sum_a W_{j,a}$,
\;goal: detect $t_j$ at least once.
...
\begin{figure*}[!t]
  ...
  \begin{subfigure}[t]{0.42\textwidth}
    ...
    \caption{Honeypot profile $\to$ zone coverage matrix.
             Green (YES) = valid deployment;
             red ($-$) = forbidden (C5).
             GENERIC is deployable in all five zones.}
    \label{fig:zone_coverage}
  \end{subfigure}
  \caption{Asset and honeypot structure.
           Panel~C: attack-load imbalance across zones.
           Panel~D: zone-coverage matrix enforced as hard
...
\begin{figure}[htbp]
  ...
  \caption{MaxSAT honeypot placement on the Large network
           ...
           DNS = yellow, AD = blue).
           Attack paths are coloured arcs; arc thickness
...
\begin{figure}[htbp]
  ...
  \caption{Technique detection results for the XXLarge
           network.
           Green bars = detected (13/18);
           red bars = undetected (5/18).
           Stealth scores $\sigma$ annotated on each bar.
           Undetected techniques are ICS-specific and require
           \texttt{scada\_trap}, which is budget-excluded at
           this scale.}
...
\begin{figure}[htbp]
  ...
  \caption{Attack path coverage heatmap for the Large network.
           Dark green $=$ 100\% coverage; orange $=$ 33\%.
           Cloud Entry and Brute Force SSH achieve 100\% in
           both directions.}
...
\begin{figure*}[!t]
  ...
  \begin{subfigure}[t]{0.58\textwidth}
    ...
    \caption{Deployed honeypot summary for the XXLarge
             network.
...
\begin{figure*}[!t]
  ...
  \caption{MaxSAT vs.\ baselines on the XLarge network
           (500,000 nodes, 5 zones, 8 profiles).
           MaxSAT-RC2 achieves $Q=67.44$ vs.\ Greedy-BiDir
           $Q=53.69$ ($+27\%$).}
...
(4) multi-round Stackelberg game.
...
\begin{figure*}[!t]
  ...
  \caption{MaxSAT placement dashboard for the XXLarge network
           (5.56M nodes, $Q=71.49$).
           Four honeypots (SSH, Database, DNS, AD) cover all
...
solver discovers and exploits automatically via the RC2
algorithm's global conflict resolution.
...
% ── Core references (in-paper citations) ─────────────────────
...
in \textit{Proc.\ 12th Int.\ Conf.\ on Integration of AI and
OR Techniques in Constraint Programming (CPAIOR)},
...
resolution,''
in \textit{Proc.\ 28th AAAI Conf.\ on Artificial
Intelligence}, pp.~2717--2723, 2014.
...
in \textit{Proc.\ 5th Annual IEEE SMC Information Assurance
Workshop}, pp.~116--122, IEEE, 2004.

\bibitem{tan2011}
G.~Tan, M.~Sherr, and W.~Zhou,
``Network-layer obliviousness: Challenges and opportunities,''
in \textit{Proc.\ 18th ACM Conf.\ on Computer and
Communications Security (CCS)}, pp.~1--12, ACM, 2011.
...
in \textit{Proc.\ 32nd Annual Computer Security Applications
Conf.\ (ACSAC)}, pp.~363--373, ACM, Dec.~2016.
...
management,''
in \textit{Proc.\ IEEE Workshop on Policies for Distributed
Systems and Networks (POLICY)}, pp.~196--206, IEEE, 2009.
...
policies,''
in \textit{Proc.\ 27th Int.\ Conf.\ on Software Engineering
(ICSE)}, pp.~196--205, ACM/IEEE, 2005.
...
in \textit{Proc.\ IEEE Int.\ Conf.\ on Communications
(ICC)}, pp.~7269--7274, IEEE, Jun.~2015.

\bibitem{bozorgi2010}
M.~Bozorgi, L.~K.~Saul, S.~Savage, and G.~M.~Voelker,
``Beyond heuristics: Learning to classify vulnerabilities and
predict exploits,''
in \textit{Proc.\ 16th ACM SIGKDD Int.\ Conf.\ on Knowledge
...
in \textit{Managing Cyber Threats: Issues, Approaches, and
Challenges}, V.~Kumar, J.~Srivastava, and A.~Lazarevic,
Eds., New York, NY, USA: Springer, 2005, pp.~247--266.

\bibitem{zhu2013}
Q.~Zhu and T.~Ba\c{s}ar,
``Game-theoretic approach to feedback-driven multi-stage
moving target defense,''
in \textit{Proc.\ 4th Int.\ Conf.\ on Decision and Game
Theory for Security (GameSec)}, Lecture Notes in Computer
Science, vol.~8252, pp.~246--263, Springer, Nov.~2013.
